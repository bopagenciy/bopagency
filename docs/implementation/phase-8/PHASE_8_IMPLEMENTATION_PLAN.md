...
**8A queda COMPLETA en su totalidad** (8A.1 + 8A.2 + 8A.3); **8B.0 (Publishing Gateway — Audit + Architecture) COMPLETE** — ver `docs/implementation/phase-8/PHASE_8B_PUBLISHING_GATEWAY_AUDIT.md`. **8B.1 (Publication Domain + Persistence) COMPLETE** — ver `docs/implementation/phase-8/PHASE_8B1_PUBLICATION_DOMAIN_PERSISTENCE_REPORT.md`. **8B.2 (Publication Application / Orchestration) COMPLETE** — ver `docs/implementation/phase-8/PHASE_8B2_APPLICATION_ORCHESTRATION_REPORT.md`. **8B.3 (Publishing Gateway Runtime / n8n / Worker) COMPLETE** — ver `docs/implementation/phase-8/PHASE_8B3_PUBLISHING_RUNTIME_N8N_WORKER_REPORT.md`. **8B.4 (Publication Web Operations / Monitoring) COMPLETE** — ver `docs/implementation/phase-8/PHASE_8B4_WEB_OPERATIONS_MONITORING_REPORT.md`. **8C (Content Calendar & Editorial Planning Layer) COMPLETE** — migración aditiva `20260828000000_phase8c_content_calendar.sql` + 4 RPCs `SECURITY DEFINER` + 1 RPC `SECURITY INVOKER` + entidad `ContentCalendarItem` + repositorio Supabase + 6 use-cases + Server Actions + UI en `/calendar` (mes + agenda) — ver `docs/implementation/phase-8/PHASE_8C_CONTENT_CALENDAR_REPORT.md`. **8D (Manual Activation Hardening) COMPLETE** — atestación humana explícita (`manual/manual`), RPCs atómicas `mark_activation_target_published` y `cancel_activation_target`, orden de locks global estandarizado `JOB -> TARGET`, 24 pruebas DB reales + 4 pruebas de concurrencia de 2 sesiones (0 deadlocks) — ver `docs/implementation/phase-8/PHASE_8D_MANUAL_ACTIVATION_HARDENING_REPORT.md`. Ver `PHASE_8_RISK_REGISTER.md` para el detalle completo.
